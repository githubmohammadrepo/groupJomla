...
class SendTo20Store
{
    ...
    public function insertOrderTable($type)
    {
        ...
        if ($type =='subsell') {
            ...
            $time = time();
            $sqlSellOrderType = "INSERT INTO pish_hikashop_order (order_user_id, order_status, order_id, order_created, order_modified, order_vendor_id)
                VALUES($this->hika_user_id, $type, $this->last_id, $time, $time,1)";
             $rows = $this->conn->query($sqlSellOrderType);
        }
    }
    /***
    ...
    public function insertCustomerVendorTable()
    {
        foreach ($this->vendor_id as $key => $VID) {
            ...
            $sqlSellOrderType = "INSERT INTO pish_customer_vendor (customer_id, vendor_id)".
            " VALUES($this->hika_user_id, $VID)";
            $rows = $this->conn->query($sqlSellOrderType);
...
$sendStore->insertOrderTable('sell');
$sendStore->insertOrderProductTable('sell');

$sendStore->insertOrderTable('subsell');




$sendStore->insertOrderProductTable('subsell');
